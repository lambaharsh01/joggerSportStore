...
function Signup() {
  ...
  const sendOtpOnEmail = () => {
    ...
    axios
      .post("/send_email_otp", { emailId: emailId })
      .then((res) => {
        Swal.fire({
      ...
      .catch((err) => {
        if (err.response.data === 409 || err.response.data === "409") {
          setSendingEmail(false);
          setBasicDetails(false);
          Swal.fire({
    ...
    <div className="container-fluid light">
      <div className="main_div col-md-8 offset-md-2 col-sm-10 offset-sm-1 bg-white">
        ...
        {basicDetails && (
          <div className="row row_sections m-2 p-2 mb-0 pb-0 bg-white">
            <div className="col-12 text-center mt-3">
              <h3 className="sec_heading">Sign Up</h3>
            </div>
            ...
            <div className="col-sm-6 py-4 mt-4">
              <div className="form-row">
                <input
                  ...
                  onFocus={(element) => addInputStyle(element)}
                  onChange={(element) =>
                    setMobileNum(element.currentTarget.value)
                  }
                  onBlur={(element) => removeInputStyle(element)}
                  onKeyDown={(elem) => {
                    const isNumericInput =
                      (elem.key >= "0" && elem.key <= "9") ||
                      elem.key === "Backspace";
                    const num_leng = elem.currentTarget.value.length;
                    if (
                      !isNumericInput ||
                      (num_leng > 9 && elem.key !== "Backspace")
                    ) {
                      elem.preventDefault();
                    }
                  }}
        ...
        {otpConfirmation && (
          <div className="row row_sections m-2 p-2 mb-0 pb-0 bg-white">
            <div className="col-12 text-center mt-3">
              <h3 className="sec_heading">Email Confirmation</h3>
            </div>
        ...
        {createPass && (
          <div className="row row_sections m-2 p-2 mb-0 pb-0 bg-white">
            <div className="col-12 text-center mt-3">
              <h3 className="sec_heading">Create Password</h3>
            </div>
