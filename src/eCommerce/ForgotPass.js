...
const navigate=useNavigate();
    ...
    useEffect(() => {
        ...
        return () => {
          document.body.style.background = 'linear-gradient(274deg, rgb(224, 235, 213) 0.00%, rgb(37, 148, 141) 100.00%)';
        };
      }, []);
...
axios.post('/send_email_for_pass_reset',{emailId:emailId})
.then((res)=>{
  Swal.fire({
...
if(err.response.data==404){
  ...
  Swal.fire({
    ...
    willClose:()=>{setEmailId(''); setBasicDetails(true);}
  });
...
const finalChange=()=>{
axios.post('/final_password_reset', {emailId:emailId, password:password, confirmPassword:confirmPassword })
...
{finalSignUpBtn &&(
<button className='signUpButton rounded-pill px-4 py-1 text-white'
onClick={finalChange}>Sign Up</button>
)}
